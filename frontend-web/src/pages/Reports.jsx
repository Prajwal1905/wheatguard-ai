...
export default function Reports() {
  ...
  const activeFilters =
    ...
    (disease !== "All" ? 1 : 0) + (severity !== "All" ? 1 : 0);
  ...
  function exportCSV() {
    ...
    saveAs(blob, `WheatGuard_Report_${new Date().toISOString().slice(0, 10)}.csv`);
  }
    ...
    <div style={styles.page}>
      ...
      <div style={styles.pageHeader}>
        ...
        <div style={styles.actions}>
          ...
          <button onClick={exportCSV} disabled={!filtered.length} style={styles.btnGhost}>
            <i className="ti ti-table-export" style={{ fontSize: 14 }} aria-hidden="true" />
            Export CSV
          </button>
          <button onClick={exportPDF} disabled={!filtered.length} style={styles.btnPrimary}>
      ...
      <div style={styles.filterBar}>
        <div style={styles.filterGroup}>
          <label style={styles.filterLabel} htmlFor="date-from">From</label>
          <input
            id="date-from"
            type="date"
            value={dateFrom}
        ...
        <div style={styles.filterGroup}>
          <label style={styles.filterLabel} htmlFor="date-to">To</label>
          <input
            id="date-to"
            type="date"
            value={dateTo}
        ...
        <div style={styles.filterGroup}>
          <label style={styles.filterLabel} htmlFor="filter-disease">Disease</label>
          <select
            id="filter-disease"
            value={disease}
        ...
        <div style={styles.filterGroup}>
          <label style={styles.filterLabel} htmlFor="filter-severity">Severity</label>
          <select
            id="filter-severity"
            value={severity}
      ...
      <div style={styles.statRow}>
        {Object.entries(severityCount).map(([sev, count]) => (
          <div key={sev} style={{ ...styles.statPill, ...severityPillStyle(sev) }}>
            {sev}: <strong>{count}</strong>
          </div>
        ))}
        {filtered.length > 0 && (
          <div style={styles.statPill}>
            Sources: {[...new Set(filtered.map((d) => d.source).filter(Boolean))].join(", ") || "—"}
          </div>
        )}
      </div>
      ...
      {filtered.length === 0 ? (
        <div style={styles.empty}>
          <i className="ti ti-chart-off" style={{ fontSize: 32, color: "#ccc", display: "block", marginBottom: 8 }} aria-hidden="true" />
        ...
        <div id="report-charts" style={styles.grid}>
          <div style={styles.card}>
            ...
            <ResponsiveContainer width="100%" height={260}>
              <PieChart>
                <Pie data={pieData} dataKey="value" nameKey="name" outerRadius={90} innerRadius={35} paddingAngle={2}>
                  {pieData.map((_, i) => (
          ...
          <div style={styles.card}>
            ...
            <ResponsiveContainer width="100%" height={260}>
              <LineChart data={trendData} margin={{ top: 4, right: 4, bottom: 0, left: -16 }}>
                ...
                <Line
                  type="monotone" dataKey="count" stroke="#1B5E20"
                  strokeWidth={2} dot={false} name="Detections"
                />
              </LineChart>
            </ResponsiveContainer>
          </div>
...
const styles = {
  ...
  btnGhost:   { display: "inline-flex", alignItems: "center", gap: 6, padding: "7px 12px", background: "transparent", border: "0.5px solid #d0d0d0", borderRadius: 7, fontSize: 13, fontWeight: 500, color: "#444", cursor: "pointer" },
  btnPrimary: { display: "inline-flex", alignItems: "center", gap: 6, padding: "7px 12px", background: "#1B5E20", border: "none", borderRadius: 7, fontSize: 13, fontWeight: 500, color: "#fff", cursor: "pointer" },
  filterBar:  { display: "flex", gap: 12, flexWrap: "wrap", alignItems: "flex-end", background: "#fff", border: "0.5px solid rgba(0,0,0,0.08)", borderRadius: 10, padding: "12px 16px", marginBottom: 16 },
  filterGroup:{ display: "flex", flexDirection: "column", gap: 4 },
  filterLabel:{ fontSize: 11, fontWeight: 500, color: "#888", textTransform: "uppercase", letterSpacing: "0.03em" },
  filterInput:{ padding: "7px 10px", borderRadius: 7, border: "0.5px solid #d0d0d0", fontSize: 13, color: "#333", background: "#fafafa", outline: "none", cursor: "pointer" },
  clearBtn:   { display: "inline-flex", alignItems: "center", gap: 5, padding: "7px 10px", background: "#FCEBEB", border: "0.5px solid #F7C1C1", color: "#A32D2D", borderRadius: 7, fontSize: 12, fontWeight: 500, cursor: "pointer", alignSelf: "flex-end" },
  statRow:    { display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 },
  statPill:   { padding: "4px 12px", borderRadius: 20, fontSize: 12, background: "#F1EFE8", color: "#555" },
  grid:       { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 },
  card:       { background: "#fff", border: "0.5px solid rgba(0,0,0,0.1)", borderRadius: 12, padding: "16px 20px" },
  cardTitle:  { fontSize: 14, fontWeight: 500, color: "#333", marginBottom: 14, display: "flex", alignItems: "center", gap: 7 },
  cardIcon:   { fontSize: 15, color: "#1B5E20" },
  empty:      { textAlign: "center", padding: "64px 20px", color: "#aaa", fontSize: 14 },
};
