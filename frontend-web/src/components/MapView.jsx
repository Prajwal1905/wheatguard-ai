...
async function fetchNdvi(lat, lon) {
  try {
    ...
    return { status: "ok", lat, lon, ndvi: data.ndvi, statusText: data.status };
  } catch {
    return {
      status: "error",
      lat,
    ...
    fields.forEach(async (f) => {
      ...
      try {
        const res = await fetch(`${API_BASE}/api/sentinel_ndvi_polygon`, {
          ...
          body: JSON.stringify({
            geometry: { coordinates: [poly.map((p) => [p[1], p[0]])] },
          }),
        });
        const data = await res.json();
        if (!cancelled) setFieldNdvi((prev) => ({ ...prev, [f.id]: data }));
      } catch {
        if (!cancelled)
          setFieldNdvi((prev) => ({
            ...prev,
      ...
      {fields.map((f) => {
        ...
        return (
          <Marker
            ...
            icon={L.divIcon({
              className: "",
              html: `<div style="width:28px;height:28px;border-radius:50%;background:${loading ? "#ccc" : ndviColor(status)};border:3px solid white;box-shadow:0 1px 4px rgba(0,0,0,0.3);display:flex;align-items:center;justify-content:center;color:white;font-size:10px;font-weight:600;">${loading ? "…" : (value ?? "-")}</div>`,
              iconSize: [28, 28],
              iconAnchor: [14, 14],
            })}
            eventHandlers={{
